...
public class SteamVrApi
{
	...
	public class ControllerDevice
	{
		...
		public Vector3 GetTrigger()
		{
			...
			object triggerEnum = FindEnum ("k_EButton_SteamVR_Trigger", enumValues);	// Old SteamVR enum name for trigger input
			if (triggerEnum == null)
				triggerEnum = FindEnum ("k_EButton_Axis1", enumValues);					// New SteamVR enum name for trigger input
	...
	public class EventProxy
	{
		...
		public void OnEventV2(int i, bool b)
		{
	...
	/** Wrapper around SteamVR_Utils.Event.Listen("device_connected", handler);
		TODO: Latest SteamVR seems to have moved this? maybe to SteamVR_Events?
		Remember to keep old version working
	 */
	public static void EventListen (string message, EventHandler handler)
	{
		...
		// First try SteamVR_Utils.Event.Listen("device_connected", handler);
		Type utilsEventType = FindTypeInAllAssemblies ("SteamVR_Utils+Event");
		if (utilsEventType != null)
		{
			// Example usage:
			//	SteamVR_Utils.Event.Listen("device_connected", eventProxy.OnEvent)

			MethodInfo listenMethod = utilsEventType.GetMethod ("Listen");
		...
		{
			// Current SteamVR interface (as of March 2017)
			// Example usage:
			//		SteamVR_Events.DeviceConnected.Listen(eventProxy.OnEvent2);

			Type eventsType = FindTypeInAllAssemblies ("SteamVR_Events");
			FieldInfo deviceConnectedField = eventsType.GetField("DeviceConnected");
			object deviceConnectedObj = deviceConnectedField.GetValue(null);
			MethodInfo listenMethod = deviceConnectedObj.GetType().GetMethod("Listen");

			ParameterInfo unityActionParmeter = listenMethod.GetParameters () [0];
			Type unityActionType = unityActionParmeter.ParameterType;

			eventProxy.proxyDelegate = Delegate.CreateDelegate (unityActionType, eventProxy, "OnEventV2");

			listenMethod.Invoke(deviceConnectedObj, new object[] { eventProxy.proxyDelegate });
		}
		return;
	}
	...
	public static void EventRemove (string message, EventHandler handler)
	{
		...
		if (utilsEventType != null)
		{
			MethodInfo removeMethod = utilsEventType.GetMethod ("Remove");
		...
		{
			// Example usage:
			// 		SteamVR_Events.DeviceConnected.Remove(UnityAction action) { this.RemoveListener(action); }

			Type eventsType = FindTypeInAllAssemblies ("SteamVR_Events");
			FieldInfo deviceConnectedField = eventsType.GetField("DeviceConnected");
			object deviceConnectedObj = deviceConnectedField.GetValue(null);
			MethodInfo removeMethod = deviceConnectedObj.GetType().GetMethod("Remove");
			removeMethod.Invoke(deviceConnectedObj, new object[] { proxy.proxyDelegate } );
		}
	}
